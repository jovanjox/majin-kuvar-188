...
import { favoritesStore, filterStore, hashStore, recentStore } from "@/lib/stores";
...
export function Home({ initialRecipeId }: { initialRecipeId?: string }) {
  const query = useSyncExternalStore(filterStore.subscribe, filterStore.get, filterStore.getServer);
  const params = useMemo(() => new URLSearchParams(query), [query]);
  const search = params.get("q") ?? "";
  const category: Category = (categories as readonly string[]).includes(params.get("k") ?? "") ? (params.get("k") as Category) : "Sve";
  const sort: SortMode = Object.hasOwn(sortLabels, params.get("red") ?? "") ? (params.get("red") as SortMode) : "preporuceno";
  const setSearch = (value: string) => filterStore.set({ q: value });
  const setCategory = (value: Category) => filterStore.set({ k: value === "Sve" ? "" : value });
  const setSort = (value: SortMode) => filterStore.set({ red: value === "preporuceno" ? "" : value });
  const initialHash = initialRecipeId ? `#recept-${initialRecipeId}` : "";
  const hash = useSyncExternalStore(hashStore.subscribe, hashStore.get, () => initialHash);
  ...
  useEffect(() => hashStore.normalizePath(), []);
  ...
  const filtered = useMemo(() => {
    ...
    const matching = recipes.filter((recipe) => {
      ...
      const searchMatch = !needle || (searchIndex.get(recipe.id) || "").includes(needle);
      ...
      <section className="collection-section" id="recepti">
        ...
        {filtered.length ? (
          ...
          <div className="empty-state">
            ...
            <button onClick={() => { filterStore.set({ q: "", k: "" }); setOnlyFavorites(false); }}>Prikaži sve recepte</button>
          </div>
        )}
      </section>
